...
inline ComponentConfig menuBarItemConfig(const std::string& label, bool isActive) {
  auto config = ComponentConfig{}
    .with_label(label)
    .with_size(ComponentSize{children(), pixels(theme::layout::MENU_BAR_HEIGHT)})
...
int renderMenuBar(UIContextT& ctx, Entity& parent, MenuBarState& state) {
  ...
  auto menuBarResult = imm::div(ctx, mk(parent),
    ComponentConfig{}
      .with_debug_name("menu_bar")
      .with_size(ComponentSize{percent(1.0f), pixels(theme::layout::MENU_BAR_HEIGHT)})
  ...
  for (size_t menuIdx = 0; menuIdx < state.menus.size(); ++menuIdx) {
    ...
    // Menu header button
    auto headerConfig = menuBarItemConfig(menu.label, isActive || menu.open);
    auto headerResult = imm::button(ctx, mk(menuBar, static_cast<int>(menuIdx)), headerConfig);
  ...
  for (size_t menuIdx = 0; menuIdx < state.menus.size(); ++menuIdx) {
    ...
    // For now, we render it as part of the flow and rely on the render system
    auto dropdownResult = imm::div(ctx, mk(parent, 1000 + static_cast<int>(menuIdx)),
      ComponentConfig{}
    ...
    for (size_t itemIdx = 0; itemIdx < menu.items.size(); ++itemIdx) {
      ...
      if (item.separator) {
        ...
        auto itemConfig = dropdownItemConfig(item.label, item.shortcut, isHovered, item.enabled);
        
        auto itemResult = imm::button(ctx, mk(dropdown, static_cast<int>(itemIdx)), itemConfig);
...
inline bool clickedOutsideMenus(const input::MousePosition& mousePos, 
                                 ...
                                 float menuBarY, float menuBarHeight) {
