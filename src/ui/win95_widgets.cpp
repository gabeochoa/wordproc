...
void DrawRaisedBorder(raylib::Rectangle rect, int thickness) {
  ...
  for (int i = 0; i < thickness; ++i) {
    // Top edge (light)
    raylib::DrawLine(x + i, y + i, x + w - i - 1, y + i, colors::BORDER_LIGHT);
    // Left edge (light)
    raylib::DrawLine(x + i, y + i, x + i, y + h - i - 1, colors::BORDER_LIGHT);
    // Bottom edge (dark)
    raylib::DrawLine(x + i, y + h - i - 1, x + w - i, y + h - i - 1, colors::BORDER_DARK);
    // Right edge (dark)
    raylib::DrawLine(x + w - i - 1, y + i, x + w - i - 1, y + h - i, colors::BORDER_DARK);
  }
}
...
void DrawSunkenBorder(raylib::Rectangle rect, int thickness) {
  ...
  for (int i = 0; i < thickness; ++i) {
    // Top edge (dark)
    raylib::DrawLine(x + i, y + i, x + w - i - 1, y + i, colors::BORDER_DARK);
    // Left edge (dark)
    raylib::DrawLine(x + i, y + i, x + i, y + h - i - 1, colors::BORDER_DARK);
    // Bottom edge (light)
    raylib::DrawLine(x + i, y + h - i - 1, x + w - i, y + h - i - 1, colors::BORDER_LIGHT);
    // Right edge (light)
    raylib::DrawLine(x + w - i - 1, y + i, x + w - i - 1, y + h - i, colors::BORDER_LIGHT);
  }
}
...
bool DrawButton(raylib::Rectangle rect, const char* text, bool enabled) {
  ...
  bool pressing = hover && raylib::IsMouseButtonDown(raylib::MOUSE_LEFT_BUTTON);
  ...
  // Draw text
  raylib::Color textColor = enabled ? colors::TEXT_COLOR : colors::TEXT_DISABLED;
  raylib::DrawText(text, textX, textY, 14, textColor);

  return clicked;
}

bool DrawCheckbox(raylib::Rectangle rect, const char* text, bool* checked, bool enabled) {
  bool changed = false;
  ...
  if (enabled && hover && raylib::IsMouseButtonReleased(raylib::MOUSE_LEFT_BUTTON)) {
    *checked = !*checked;
    changed = true;
  }
...
int DrawMenuBar(std::vector<Menu>& menus, int menuBarY, int menuBarHeight) {
  ...
  raylib::Vector2 mousePos = raylib::GetMousePosition();
  bool mouseInMenuBar = mousePos.y >= menuBarY && mousePos.y < menuBarY + menuBarHeight;
  ...
  for (std::size_t i = 0; i < menus.size(); ++i) {
    ...
    menu.bounds = {static_cast<float>(x), static_cast<float>(menuBarY),
                   static_cast<float>(itemWidth), static_cast<float>(menuBarHeight)};

    bool hover = raylib::CheckCollisionPointRec(mousePos, menu.bounds);
    bool clicked = hover && raylib::IsMouseButtonPressed(raylib::MOUSE_LEFT_BUTTON);
    ...
    if (menu.open || hover) {
      ...
      raylib::DrawText(menu.label.c_str(), x + 8, menuBarY + 3, 14, colors::TITLE_TEXT);
    } else {
      raylib::DrawText(menu.label.c_str(), x + 8, menuBarY + 3, 14, colors::TEXT_COLOR);
    }

    // Draw dropdown if open
    if (menu.open) {
      int selectedItem = DrawDropdownMenu(menu, x, menuBarY + menuBarHeight, 20);
      if (selectedItem >= 0) {
        clickedMenu = static_cast<int>(i * 100 + selectedItem);  // Encode menu and item
  ...
  if (raylib::IsMouseButtonPressed(raylib::MOUSE_LEFT_BUTTON) && !mouseInMenuBar) {
    bool clickedInDropdown = false;
    for (const auto& menu : menus) {
      if (menu.open) {
        // Check if click is in dropdown area
        int dropdownHeight = static_cast<int>(menu.items.size()) * 20;
        raylib::Rectangle dropdownRect = {menu.bounds.x, menu.bounds.y + menu.bounds.height,
                                          150, static_cast<float>(dropdownHeight)};
...
int DrawDropdownMenu(Menu& menu, int x, int y, int itemHeight) {
  ...
  raylib::Rectangle dropdownRect = {static_cast<float>(x), static_cast<float>(y),
  ...
  for (std::size_t i = 0; i < menu.items.size(); ++i) {
    ...
    raylib::Rectangle itemRect = {static_cast<float>(x + 2), static_cast<float>(itemY),
                                  static_cast<float>(maxWidth - 4), static_cast<float>(itemHeight)};

    if (item.separator) {
      // Draw separator line
      int sepY = itemY + itemHeight / 2;
      raylib::DrawLine(x + 4, sepY, x + maxWidth - 4, sepY, colors::BORDER_DARK);
      raylib::DrawLine(x + 4, sepY + 1, x + maxWidth - 4, sepY + 1, colors::BORDER_LIGHT);
    } else {
      bool hover = raylib::CheckCollisionPointRec(mousePos, itemRect) && item.enabled;

      if (hover) {
        raylib::DrawRectangleRec(itemRect, colors::MENU_HIGHLIGHT);
        raylib::DrawText(item.label.c_str(), x + 8, itemY + 3, 14, colors::TITLE_TEXT);
        if (!item.shortcut.empty()) {
          int shortcutX = x + maxWidth - raylib::MeasureText(item.shortcut.c_str(), 14) - 12;
          raylib::DrawText(item.shortcut.c_str(), shortcutX, itemY + 3, 14, colors::TITLE_TEXT);
        }
        ...
        raylib::Color textColor = item.enabled ? colors::TEXT_COLOR : colors::TEXT_DISABLED;
        raylib::DrawText(item.label.c_str(), x + 8, itemY + 3, 14, textColor);
        if (!item.shortcut.empty()) {
          int shortcutX = x + maxWidth - raylib::MeasureText(item.shortcut.c_str(), 14) - 12;
          raylib::DrawText(item.shortcut.c_str(), shortcutX, itemY + 3, 14, textColor);
        }
      }
    }
...
int DrawMessageDialog(raylib::Rectangle dialogRect, const char* title,
  ...
  int buttonWidth = 75;
  int buttonHeight = 23;
  int buttonY = static_cast<int>(dialogRect.y + dialogRect.height) - buttonHeight - 12;

  if (hasCancel) {
    int okX = static_cast<int>(dialogRect.x + dialogRect.width) - 2 * buttonWidth - 24;
    int cancelX = static_cast<int>(dialogRect.x + dialogRect.width) - buttonWidth - 12;
    ...
    if (DrawButton({static_cast<float>(cancelX), static_cast<float>(buttonY),
                    static_cast<float>(buttonWidth), static_cast<float>(buttonHeight)},
                   "Cancel", true)) {
      result = 1;
    }
  } else {
    int okX = static_cast<int>(dialogRect.x + (dialogRect.width - buttonWidth) / 2);
    if (DrawButton({static_cast<float>(okX), static_cast<float>(buttonY),
                    static_cast<float>(buttonWidth), static_cast<float>(buttonHeight)},
                   "OK", true)) {
      result = 0;
