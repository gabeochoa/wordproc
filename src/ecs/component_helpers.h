...
inline void set(StatusComponent& status, const std::string& msg, bool error = false) {
    status.text = msg;
...
inline void updateLayout(LayoutComponent& layout, int w, int h) {
    ...
    layout.menuBar = {0, layout.titleBarHeight, static_cast<float>(w), layout.menuBarHeight};
    layout.statusBar = {0, static_cast<float>(h - layout.statusBarHeight),
                       static_cast<float>(w), layout.statusBarHeight};
    
    float textTop = layout.titleBarHeight + layout.menuBarHeight + layout.borderWidth;
    float textHeight = static_cast<float>(h) - layout.titleBarHeight - layout.menuBarHeight -
                       layout.statusBarHeight - 2 * layout.borderWidth;
    layout.textArea = {layout.borderWidth, textTop,
                      static_cast<float>(w) - 2 * layout.borderWidth, textHeight};
    
    // Calculate page display dimensions for paged mode
    if (layout.pageMode == PageMode::Paged) {
        float availableWidth = layout.textArea.width - 20.0f;  // Margin for page shadow
        ...
        layout.pageOffsetX = layout.textArea.x + (layout.textArea.width - layout.pageDisplayWidth) / 2.0f;
    }
}
...
inline LayoutComponent::Rect effectiveTextArea(const LayoutComponent& layout) {
    if (layout.pageMode == PageMode::Pageless) {
        ...
        if (layout.lineWidthLimit > 0.0f) {
            float limitedWidth = layout.lineWidthLimit * 8.0f;  // Approximate char width
            if (limitedWidth < layout.textArea.width) {
                float offset = (layout.textArea.width - limitedWidth) / 2.0f;
                return {layout.textArea.x + offset, layout.textArea.y, limitedWidth, layout.textArea.height};
            }
        }
    ...
    return {
        layout.pageOffsetX + marginScaled,
...
inline void togglePageMode(LayoutComponent& layout) {
    layout.pageMode = (layout.pageMode == PageMode::Pageless) ? PageMode::Paged : PageMode::Pageless;
