...
struct LayoutComponent : public afterhours::BaseComponent {
    ...
    void updateLayout(int w, int h) {
        ...
        float textHeight = static_cast<float>(h) - titleBarHeight -
                           menuBarHeight - statusBarHeight - 2 * borderWidth;
        ...
        if (pageMode == PageMode::Paged) {
            float availableWidth =
                textArea.width - 20.0f;  // Margin for page shadow
            ...
            pageOffsetX =
                textArea.x + (textArea.width - pageDisplayWidth) / 2.0f;
        }
    }
    ...
    Rect effectiveTextArea() const {
        if (pageMode == PageMode::Pageless) {
            ...
            if (lineWidthLimit > 0.0f) {
                float limitedWidth =
                    lineWidthLimit * 8.0f;  // Approximate char width
                if (limitedWidth < textArea.width) {
                    float offset = (textArea.width - limitedWidth) / 2.0f;
                    return {textArea.x + offset, textArea.y, limitedWidth,
                            textArea.height};
                }
            }
            return textArea;
        ...
        return {pageOffsetX + marginScaled,
    ...
    void togglePageMode() {
        pageMode = (pageMode == PageMode::Pageless) ? PageMode::Paged
                                                    : PageMode::Pageless;
        updateLayout(screenWidth, screenHeight);
    }

    // Set line width limit (0 to disable)
    void setLineWidthLimit(float chars) { lineWidthLimit = chars; }
};
